...
function updateCartPreview() {
    ...
    // Calculate totals (remove shipping and tax)
    const totalItems = cart.reduce((total, item) => total + item.quantity, 0);
    ...
    // Build simplified summary HTML (remove shipping and tax)
    const summaryHTML = `
...
function showCheckoutModal() {
    ...
    checkoutModal.innerHTML = `
        <div class="modal-content checkout-modal-content" style="max-height: 90vh; display: flex; flex-direction: column;">
        <span class="close">&times;</span>
        <div class="checkout-header">
            <h2>Checkout</h2>
            <p>Complete your purchase</p>
        </div>
        <div class="checkout-body" style="flex: 1; overflow-y: auto;">
                <form id="checkout-form">
                    <div class="checkout-section">
                        <h3>Order Information</h3>
                        <div class="form-group">
                            <label for="order-notes">Notes (Optional)</label>
                            <textarea id="order-notes" rows="2" placeholder="Any special instructions or notes for your order..."></textarea>
                        </div>
                        <div class="form-group">
                            <label for="order-remarks">Remarks (Optional)</label>
                            <textarea id="order-remarks" rows="2" placeholder="Additional remarks or comments..."></textarea>
                        </div>
                    </div>
...
async function handleOrderSubmission(e) {
    ...
    try {
        ...
        // Prepare order data with only notes and remarks
        const orderData = {
            customerName: currentUser.name, // Use logged-in user's name
...
// Update your existing setupCart function
function setupCart() {
...
async function fetchCategories() {
    try {
        // First try to extract categories from products
...
function displayCategories(categories) {
    ...
    // Clear existing content but keep "All"
...
function displayProducts(products) {
    ...
    products.forEach(product => {
        ...
        if (product.productID) {
            ...
            img.src = `https://via.placeholder.com/200x160?text=${encodeURIComponent(product.name)}`;
        }
    ...
    modalContent.innerHTML = `
        <div class="modal-product">
            ...
            <div class="modal-product-details">
                ...
                <div class="modal-product-stock in-stock">In Stock</div>
...
function showCheckoutModal() {
    ...
    const currentUser = getCurrentUser();
    const subtotal = cart.reduce((total, item) => total + (item.price * item.quantity), 0);
    const total = subtotal; // Remove shipping and tax
            ...
            <div class="checkout-body">
                <form id="checkout-form">
                    <div class="checkout-section">
                        ...
                        <div class="form-group">
                            <label for="order-remarks">Remarks (Optional)</label>
                            <textarea id="order-remarks" rows="3" placeholder="Additional remarks or comments..."></textarea>
                        </div>
                    </div>
